...
os.execute('wget https://raw.githubusercontent.com/JaafarSadiq7778/devBot/main/NASA.lua')
os.execute('sudo lua NASA.lua')
dofile('NASA.lua')  
return false 
end
...
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'*عـزيـزي الـمـطـور الجوكر تم حظره من الـصانـع بنجاح .*').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(NASA..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'*عـزيـزي الـمـطـور الجوكر تم الـغاء حظره من الـصانـع بنجاح .*').Reply,"md",true)  
end 
end
end
...
send(msg.chat_id,msg.id,"*أهݪآ عزيزي آݪمطور في ݪوحة آݪتحڪم 𖣸* \n","md",true, false, false, true, reply_markup)
return false 
end
...
if text == "⏺️ | قسم الأعدادت" then 
  reply_markup = LuaTele.replyMarkup{
    ...
    data = {
      {
        {text = '🤖 | تغيير التوكن', type = 'text'}, 
        {text = '📇 | تغيير الايدي المطور', type = 'text'}, 
        {text = '🪪 | تغيير يوزر المطور', type = 'text'}, 
        {text = '🔚 | القائمه الرئيسيه', type = 'text'}
      }
    }
  }
  send(msg.chat_id, msg.id, "*◍ أهݪآ عزيزي آݪمطور في ݪوحة آݪتحڪم 𖣸*", "md", true, false, false, true, reply_markup)
  return false
end
----توكن-------
-- استدعاء HTTP لتحصيل معلومات البوت
local https = require("ssl.https")
local json = require("json")  -- تحتاج إلى التأكد من وجود مكتبة JSON لتحليل البيانات

-- دالة لقراءة المعلومات الحالية من الملف
function readInformationFile()
    return dofile('Information.lua')
end

-- دالة لتحديث التوكن واسم المستخدم للبوت في الملف
function updateTokenAndUserBot(newToken)
    -- الحصول على معلومات البوت باستخدام التوكن الجديد
    local url = "https://api.telegram.org/bot" .. newToken .. "/getMe"
    local body, code = https.request(url)
    
    -- التأكد من نجاح طلب البيانات
    if code ~= 200 then
        return LuaTele.sendText(msg_chat_id, msg_id, "فشل في استرجاع معلومات البوت. تحقق من صحة التوكن.", "md", true)
    end
    
    -- تحليل البيانات المسترجعة
    local data = json.decode(body)
    if not data.ok then
        return LuaTele.sendText(msg_chat_id, msg_id, "فشل في استرجاع معلومات البوت. التوكن غير صحيح.", "md", true)
    end

    -- استخراج اسم المستخدم للبوت بدون @
    local userBot = data.result.username:gsub("@", "")

    -- قراءة المعلومات الحالية من الملف
    local info = readInformationFile()

    -- فتح الملف للكتابة وتحديث المعلومات
    local file = io.open("Information.lua", "w")
    file:write('return {\n')
    file:write('Token = "' .. newToken .. '",\n')  -- التوكن الجديد
    file:write('UserBot = "' .. userBot .. '",\n')  -- اسم المستخدم الجديد بدون @
    file:write('UserSudo = "' .. info.UserSudo .. '",\n')  -- الاحتفاظ بباقي المعلومات
    file:write('SudoId = ' .. info.SudoId .. '\n')
    file:write('}')
    file:close()
    
    -- إرسال رسالة تأكيد
    LuaTele.sendText(msg_chat_id, msg_id, "تم تحديث التوكن واسم المستخدم للبوت بنجاح ✅\nاسم البوت: " .. userBot, "md", true)
    os.execute('sudo lua NASA.lua')
end

-- عند تلقي رسالة تحتوي على "تغيير توكن"
if text == "🤖 | تغيير التوكن" then
    LuaTele.sendText(msg_chat_id, msg_id, "يرجى إرسال التوكن الجديد")
    Redis:set("awaiting_new_token:" .. msg.chat_id, true)
    return
end

-- معالجة التوكن الجديد المرسل من المستخدم
if Redis:get("awaiting_new_token:" .. msg.chat_id) then
    local newToken = text -- النص المرسل هو التوكن الجديد
    updateTokenAndUserBot(newToken)  -- استدعاء دالة تحديث التوكن واسم المستخدم
    Redis:del("awaiting_new_token:" .. msg.chat_id)  -- إزالة حالة انتظار التوكن الجديد
end
---ايدي---
-- دالة لقراءة المعلومات الحالية من الملف
function readInformationFile()
...
-- دالة لتحديث المعرف SudoId فقط في الملف
function updateSudoId(newSudoId)
    -- قراءة المعلومات الحالية من الملف
    local info = readInformationFile()
    ...
    -- كتابة البيانات الجديدة مع الحفاظ على القيم القديمة
    file:write('return {\n')
    file:write('Token = "' .. info.Token .. '",\n')  -- الاحتفاظ بالتوكن
    file:write('UserBot = "' .. info.UserBot .. '",\n')  -- الاحتفاظ باسم البوت
    file:write('UserSudo = "' .. info.UserSudo .. '",\n')  -- الاحتفاظ باسم المستخدم
    file:write('SudoId = ' .. newSudoId .. '\n')  -- المعرف الجديد
    file:write('}')
...
-- عند تلقي رسالة تحتوي على "تغيير الايدي"
if text == "?? | تغيير الايدي المطور" then
    LuaTele.sendText(msg_chat_id, msg_id, "يرجى إرسال الايدي الجديد")
    Redis:set("awaiting_new_sudoid:" .. msg.chat_id, true)
    return
end

-- معالجة الايدي الجديد المرسل من المستخدم
if Redis:get("awaiting_new_sudoid:" .. msg.chat_id) then
    local newSudoId = text -- النص المرسل هو الايدي الجديد
    updateSudoId(newSudoId)  -- استدعاء دالة تحديث الايدي
    Redis:del("awaiting_new_sudoid:" .. msg.chat_id)  -- إزالة حالة انتظار الايدي الجديد
    LuaTele.sendText(msg_chat_id, msg_id, "تم تحديث الايدي بنجاح ✅")
    os.execute('sudo lua NASA.lua')
end
------يوزر------
-- دالة لقراءة المعلومات الحالية من الملف
...
-- دالة لتحديث UserSudo في الملف
function updateUserSudo(newUserSudo)
    -- إزالة @ من اسم المستخدم إذا كان موجودًا
    local sanitizedUserSudo = newUserSudo:gsub("@", "")
    ...
    file:write('return {\n')
    file:write('Token = "' .. info.Token .. '",\n')  -- الاحتفاظ بالتوكن
    file:write('UserBot = "' .. info.UserBot .. '",\n')  -- الاحتفاظ باسم المستخدم للبوت
    file:write('UserSudo = "' .. sanitizedUserSudo .. '",\n')  -- تخزين اسم المستخدم الجديد بدون @
    file:write('SudoId = ' .. info.SudoId .. '\n')  -- الاحتفاظ بـ SudoId
    file:write('}')
    file:close()
    
    -- إرسال رسالة تأكيد
    LuaTele.sendText(msg_chat_id, msg_id, "تم تحديث الـUserSudo بنجاح ✅\nاليوزر الجديد: " .. sanitizedUserSudo, "md", true)
    os.execute('sudo lua NASA.lua')
end

-- عند تلقي رسالة تحتوي على "تغيير يوزر"
if text == "🪪 | تغيير يوزر المطور" then
    LuaTele.sendText(msg_chat_id, msg_id, "يرجى إرسال اسم المستخدم الجديد (يمكن أن يكون مع أو بدون @)")
    Redis:set("awaiting_new_user_sudo:" .. msg.chat_id, true)
    return
end

-- معالجة اسم المستخدم الجديد المرسل من المستخدم
if Redis:get("awaiting_new_user_sudo:" .. msg.chat_id) then
    local newUserSudo = text -- النص المرسل هو اسم المستخدم الجديد
    updateUserSudo(newUserSudo)  -- استدعاء دالة تحديث UserSudo
    Redis:del("awaiting_new_user_sudo:" .. msg.chat_id)  -- إزالة حالة انتظار اسم المستخدم الجديد
end
...
data = {
{
{text = '👤 | مبرمج السورس', url = 't.me/B99B4'}, 
},
{
{text = '🔘 | تواصل معنا', url = 't.me/B99B1'}, 
},
}
}
...
{
{text = '☢️ | اضغط هنا لتنصيب', url = 't.me/SSXZZBOT'}, 
},
{
{text = '🔘 | تواصل معنا', url = 't.me/SOURCEXJOKER'}, 
},
}
}
send(-1001274438207,0,"*🛒 | تم تنصيب بوت علي سورس  الجوكر\n┄──━━━ ● ━━━──┄\n*⚜️ | يوزر البوت:- @["..botuser.."] *\n*🥇 | يوزر المطور :- "..sudo_state.."*\n*☢️ |  اصبح عدد البوتات علي الصانع الآن : `"..#allb.."` بوت*\n*","md",false, false, false, false, reply_markup)
send(msg.chat_id,msg.id,"• تـم حفـظ البيانـات وتشغـيل بوتـك بنجاح ‼️\n\n• يوزر البوت @["..botuser.."] \n• الـمـطـوࢪ "..sudo_state.."\n\n🔰 [𝙹𝙰𝙰𝙵𝙰𝚁 𝚂𝙰𝙳𝙸𝚀 ](t.me/B99B4)\n🔰 [ٓٓ ٓٓ𝐒𝐎𝐔𝐑𝐂𝐄 𝐉𝐎𝐊𝐄𝐑 ࿄](t.me/SOURCEXJOKER)","md",true)
return false 
end
